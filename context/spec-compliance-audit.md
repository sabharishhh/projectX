...
| **Merge never happens automatically** | `memory-engine-spec.md §4` | Spec calls for silent auto-merge on clean additions as ordinary background behavior. In reality, `merge_preview`/`merge_apply` are only ever invoked manually via their dedicated endpoints (tested via curl). Branches accumulate indefinitely with zero automatic reconciliation between them. This is the single largest structural gap against spec. |
