...
def resolve_forget(req: ForgetResolveRequest):
    ...
    if req.choice == "soft":
        ok = forget_unit(p["hash"], p["source"], p["branch"], "user asked to forget this")
        if ok:
            ledger.log("memory_forgotten", f"soft-forgot: {p['content']}", p["source"], actor="user")
    elif req.choice == "hard":
        ok = forget_unit(p["hash"], p["source"], p["branch"], "user asked to permanently delete this")
        ok = purge_unit(p["hash"]) and ok
        if ok:
